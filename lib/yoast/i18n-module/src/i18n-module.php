...
/**
 * This file is cloned from yoast/i18n-module@2.0.1
 *
 * @see https://github.com/Yoast/i18n-module/commit/c08da78bba4747dff960e31dda2be46aae6b30e8
 * @see https://github.com/Yoast/i18n-module
 */
...
class Yoast_I18n_v2 {
    ...
    private function promo_message() {
        ...
        if ( $this->translation_exists && $this->translation_loaded && $this->percent_translated < 90 ) {
            $message = __( 'As you can see, there is a translation of this plugin in %1$s. This translation is currently %3$d%% complete. We need your help to make it complete and to fix any errors. Please register at %4$s to help complete the translation to %1$s!', $this->textdomain, 'wp-cloudflare-guard' );
        } else if ( ! $this->translation_loaded && $this->translation_exists ) {
            $message = __( 'You\'re using WordPress in %1$s. While %2$s has been translated to %1$s for %3$d%%, it\'s not been shipped with the plugin yet. You can help! Register at %4$s to help complete the translation to %1$s!', $this->textdomain, 'wp-cloudflare-guard' );
        } else if ( ! $this->translation_exists ) {
            $message = __( 'You\'re using WordPress in a language we don\'t support yet. We\'d love for %2$s to be translated in that language too, but unfortunately, it isn\'t right now. You can change that! Register at %4$s to help translate it!', $this->textdomain, 'wp-cloudflare-guard' );
        }
    ...
    public function promo() {
        ...
        if ( $message ) {
            ...
            echo '<div>';
            echo '<h2>' . sprintf( __( 'Translation of %s', $this->textdomain, 'wp-cloudflare-guard' ), $this->plugin_name ) . '</h2>';
            ...
            echo '<p>' . $message . '</p>';
            echo '<p><a href="' . esc_url( $this->register_url ) . '">' . __( 'Register now &raquo;', $this->textdomain, 'wp-cloudflare-guard' ) . '</a></p>';
            echo '</div>';
            echo '</div>';
        }
    }
